class GameLoop_old {
  ...
  #renderDynamicBackgrounds=()=>{
    ...
    if (BackgroundHandler_old.backgroundRemoveQueue.length > 0) {
      for (let i = 0; i < BackgroundHandler_old.backgroundRemoveQueue.length; i++) {
        BackgroundHandler_old.dynamicBackgrounds.splice(BackgroundHandler_old.dynamicBackgrounds.findIndex(item => item.id === BackgroundHandler_old.backgroundRemoveQueue[i]), 1);
      }
      BackgroundHandler_old.backgroundRemoveQueue = [];
      console.log("backgrounds:", BackgroundHandler_old.dynamicBackgrounds);
    }
    ...
    for (let i = 0, len = BackgroundHandler_old.dynamicBackgrounds.length; i < len; i++) {
      try {
        BackgroundHandler_old.dynamicBackgrounds[i].render();
  ...
  #renderStaticBackgrounds=()=>{
    ...
    for (let i = 0, len = BackgroundHandler_old.staticBackgrounds.length; i < len; i++) {
      try {
        BackgroundHandler_old.staticBackgrounds[i].render();
      } catch(e){console.error(e)}
    }
  }
